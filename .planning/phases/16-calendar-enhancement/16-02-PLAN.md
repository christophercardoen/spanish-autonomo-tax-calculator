...
must_haves:
  ...
  key_links:
    ...
    - from: "renderCalendar"
      to: "CalendarManager.getDaysForMonth"
      via: "load days data at render start"
      pattern: "CalendarManager\\.getDaysForMonth"
---
...
1. At the START of renderCalendar(), load days for current month via CalendarManager.getDaysForMonth(year, month).
   Store result in a local variable (e.g., `monthDays`) for use in rendering.
...
<task type="auto">
  <name>Task 4: Add Bulk Tag Selected Days with Client Feature (CALENDAR-07)</name>
  <files>autonomo_dashboard.html</files>
  <action>
The calendar already has week selectors (line 8082, class="week-selector") and day selection via shift-click.
The existing toolbar (lines 7046-7066) only allows setting location status.

Add ability to bulk-tag selected days with a client/project:

1. Add "Tag with Client" button to the toolbar (after existing location buttons, around line 7064):
   ```html
   <button type="button" class="toolbar-btn" onclick="openBulkTagModal()" title="Tag selected days with client">
     Tag with Client
   </button>
   ```

2. Create Bulk Tag Modal (add after dayEditorModal):
   ```html
   <!-- Bulk Tag Modal -->
   <div id="bulkTagModal" class="modal" style="display: none;">
     <div class="modal-content" style="max-width: 400px;">
       <div class="modal-header">
         <h3>Tag Selected Days</h3>
         <button type="button" class="modal-close" onclick="closeBulkTagModal()">&times;</button>
       </div>
       <form id="bulkTagForm" onsubmit="executeBulkTag(event)">
         <div class="form-group">
           <label>Selected days: <span id="bulkTagCount">0</span></label>
         </div>
         ...
         <div class="form-group">
           <label>
             ...
             Overwrite existing client tags
           </label>
...
3. Implement JavaScript functions:
   ...
   a. openBulkTagModal():
      - Check if any days selected (selectedDates.size > 0)
      - If none selected, show alert "Select days first (use shift+click or week selectors)"
      - Set bulkTagCount to selectedDates.size
      - Populate client dropdown with ClientManager.getClients()
      - Reset project dropdown to disabled
      - Show modal
   ...
   c. updateBulkTagProjects():
      - Get selected client_id
      - If none, disable project dropdown
      - Otherwise fetch projects via ProjectManager.getProjectsByClient(clientId)
      - Populate and enable dropdown

   d. executeBulkTag(event):
      - event.preventDefault()
      - Get client_id, project_id, overwrite flag
      - Loop through selectedDates
      - For each date:
        - If overwrite=false and day already has client_id, skip
        - Otherwise update via CalendarManager.saveDay(dateKey, { client_id, project_id })
      - Close modal
      - Clear selection
      - Re-render calendar
      - Show feedback: "Tagged N days (skipped M existing)"
  </action>
  <verify>
Test flow:
1. Click week selector (W button) to select a week
2. Click "Tag with Client" button in toolbar
3. Modal opens showing "7" selected days
4. Select client from dropdown
5. Select project from dropdown
6. Click Apply Tags
7. Calendar shows client abbreviations on tagged days
8. Repeat with some days already tagged + overwrite unchecked - verify skip message
  </verify>
  <done>
Bulk tag feature allows tagging multiple selected days with client/project in one action (CALENDAR-07)
  </done>
</task>
...
7. Week selectors and shift-click allow multi-day selection
8. Bulk tag modal allows tagging selected days with client/project
</verification>
...
- Bulk day selection with client tagging works (CALENDAR-07)
</success_criteria>
