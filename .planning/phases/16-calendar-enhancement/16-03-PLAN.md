...
1. The project modal is `id="project-modal"` (line 6774) with form `id="project-form"` (line 6781).
   Key element IDs:
   - project-client-id (hidden input)
   - project-name (text input)
   - project-rate-type (select)
   - project-rate (number input)
   - project-start, project-end (date inputs)
   - project-status-override (select)
   - project-notes (textarea)
   - project-submit-btn (submit button)
   The form is managed by ProjectFormUI object (line 17913).

2. Add work pattern section AFTER the notes field (around line 6831, after project-notes textarea).
   Insert the fieldset inside the form, before the modal-actions div:
   ```html
...
4. Modify ProjectFormUI.handleSubmit() (around line 17968) to include work_pattern:
   - Collect checked days from patternDay checkboxes
   - Collect first_week_extra from patternFirstWeek checkboxes
   - Get location from projectPatternLocation dropdown
   - Build pattern object: { type: 'weekly', days: [...], first_week_extra: [...], location: '...', active: boolean }
   - Include work_pattern field in project data passed to ProjectManager.createProject() or updateProject()

5. Modify ProjectFormUI.open() (around line 17943-17954) to populate pattern fields when editing:
   - If project.work_pattern exists, check projectPatternActive checkbox
   - Show projectPatternFields div if active
   - Check appropriate patternDay checkboxes based on work_pattern.days array
   - Check appropriate patternFirstWeek checkboxes based on work_pattern.first_week_extra array
   - Select correct location in projectPatternLocation dropdown
