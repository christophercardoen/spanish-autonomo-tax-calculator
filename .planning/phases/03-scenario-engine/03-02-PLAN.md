...
must_haves:
  truths:
    - "Edit button on card opens modal dialog"
    - "Modal shows split view: inputs on left, live results on right"
    - "Typing in any input field instantly updates calculated results"
    - "All scenario values are editable including fiscal overrides (RETA, minimos)"
    - "Save button closes modal and updates card; Cancel discards changes"
    - "Pressing Escape cancels without saving"
  artifacts:
    - path: "autonomo_dashboard.html"
      provides: "Native HTML dialog element with split view edit form"
  ...
  key_links:
    - from: "openEditModal"
      ...
      pattern: "showModal"
    - from: "input events"
      to: "live results pane"
      via: "requestAnimationFrame debounced onScenarioInputChange"
      pattern: "requestAnimationFrame"
---

<objective>
Implement edit modal dialog with split view (inputs left, live results right) and instant recalculation as user types.
...
10. Edit again, expand "Fiscal Overrides", enter 500 for RETA Monthly, verify results update instantly with lower tax
  ...
  <done>
Live recalculation works on every keystroke with requestAnimationFrame debounce. Results pane shows full calculation breakdown. Save updates scenario and re-renders cards. Delete works for custom scenarios. Fiscal overrides enable what-if analysis.
  </done>
</task>
...
- Results show full calculation breakdown
