...
must_haves:
  ...
  key_links:
    ...
    - from: "calculateFullIRPF"
      to: "scenario results"
      via: "calculateScenarioResults() wrapper"
      pattern: "calculateScenarioResults"
---
...
function calculateScenarioResults(scenario) {
  ...
  // Reuse Phase 1 calculation with fiscal config
...
**Add calculateFullIRPFWithFiscal variant (copy of calculateFullIRPF but accepts fiscal param):**
This function is identical to calculateFullIRPF but uses the `fiscal` parameter instead of directly referencing FISCAL_2025. This allows scenarios to override RETA, minimos, brackets for what-if analysis.
...
```
  ...
  <verify>
Browser console: `typeof SCENARIO_PRESETS === 'object'` returns true. `Object.keys(SCENARIO_PRESETS).length === 5` returns true. After page load, `scenarioState.scenarios.A.monthlyRevenue === 3000` returns true.
  </verify>
  <done>
SCENARIO_PRESETS constant exists with A-E frozen objects. State management loads/saves to localStorage. calculateScenarioResults() wraps Phase 1 calculation with fiscal overrides support. getSortedScenarios() returns scenarios sorted by leefgeld descending.
  </done>
</task>
